  ...
  >
    <template #title>
      <a-row type="flex">
        ...
        >
          <a-button type="primary" @click="showDrawer" class="mx-2"
            >Add New Rider</a-button
          >
        </a-col>
      </a-row>
    </template>
    <a-table :columns="columns" :data-source="data" :pagination="false">
                ...
                <a-form-item label="Motor Bike">
                  <a-input
                    v-decorator="[
                      'bike',
                      {
                        initialValue: currentRecord.bike,
                        rules: [
      ...
      <template slot="deleteBtn" slot-scope="text, row">
        <a-button
        icon="delete"
          type="link"
          :data-id="row.key"
...
import { addRider, deleteRider, updateRider } from "../../database/firestore";
...
export default {
  ...
  methods: {
    showModal(record) {
      ...
      this.mdoalvisible = true;
    },
    handleOk(e) {
      this.loading = true;
      console.log(e);
      setTimeout(() => {
        this.mdoalvisible = false;
        this.loading = false;
      }, 3000);
    },
    handleCancel() {
    ...
    handleEdit(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          updateRider(this.currentRecord.id, payload).then(() => {
            location.reload();
          });
        }
      });
