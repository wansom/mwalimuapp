<template>
  <div>
    <a-table :columns="columns" :data-source="orders"  :rowKey="record => record.id">
    <a slot="name" slot-scope="text">{{ text }}</a>
    <template slot="editBtn" slot-scope="text,row">
      <a-modal v-model="visible"  :data-id="row.id" :title="'Order No:'+ currentRecord.id" on-ok="handleOk" v-if="currentRecord">
      <template slot="footer">
        <a-button key="back" @click="handleCancel">
          Close
        </a-button>
        <a-button key="submit" type="primary" :loading="loading" @click="()=>{handleSubmit(currentRecord.id,currentRecord.userId)}">
          Submit
        </a-button>
      </template>
    ...
    <a-form-item label="Select Rider">
      ...
      >
        <a-select-option  v-for="(rider,index) of riders" :key="index" :value="rider.id">
          {{ rider.name }}
        </a-select-option>
      </a-select>
    </a-form-item>
				...
				<a-button type="link" :data-id="row.key" @click="()=>{showModal(row)}">
